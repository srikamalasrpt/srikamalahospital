...
import { Menu, X, Phone, User, LogIn, ChevronRight, ArrowLeft } from 'lucide-react';
    ...
    <nav className={`fixed w-full z-[100] transition-all duration-700 px-4 md:px-8 py-2 ${isScrolled ? 'top-0' : 'top-2'}`}>
      <div className={`container mx-auto max-w-7xl glass-card px-5 py-2 flex justify-between items-center ${isScrolled ? '!rounded-full !bg-white/95 border-white/60 shadow-lg' : ''}`}>
        ...
        {isMainPage ? (
          ...
          <button onClick={() => navigate(-1)} className="flex items-center gap-3 group text-hospital-dark hover:text-hospital-primary transition-all">
            <div className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center group-hover:-translate-x-1 transition-transform">
              <ArrowLeft size={16} />
            </div>
            <div className="text-left flex flex-col justify-center">
              <h1 className="text-sm font-black tracking-tight leading-none font-['Noto_Sans_Telugu']">
                వెనక్కి వెళ్ళండి
              </h1>
              <p className="text-[7px] uppercase font-bold tracking-[0.2em] text-gray-400 leading-none mt-1">GO BACK</p>
            </div>
          </button>
        )}

        {/* Desktop Navigation (Only on Main Page) */}
        {isMainPage && (
        ...
        {/* Mobile Toggle (Only on Main Page) */}
        {isMainPage && (
